...
func NewNTreeCLI(n *NTree) *cli.CLI {
	...
	cmdStart.AddFlag("config", "c", "file", "Config file", cli.TypePathFile|cli.MustExist, nil)
	cmdStart.AddFlag("workdir", "w", "dir", "Working directory", cli.TypePathFile|cli.Required, nil)
	cmdStart.AddFlag("rootdir", "r", "dir", "Root directory", cli.TypePathFile|cli.Required, nil)

	cmdMsg := nTreeCLI.AddCmd("send", "Sends command to already running agent", getCLISendHandler(n))
	cmdMsg.AddFlag("config", "c", "file", "Config file", cli.TypePathFile|cli.MustExist, nil)
